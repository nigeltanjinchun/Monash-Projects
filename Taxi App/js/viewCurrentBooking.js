// The name of this file is viewCurrentBooking.js
// The function of this JavaScript file is to connect with viewCurrentBooking.html
...
let pageRef = document.getElementById("current-content");
...
output += `<span>${getLocationName(bookings.currentLocation)}</span></span></h5></div>`;
output += `<br><hr>`;
...
for (let i = 0; i < bookings.stops.length; i++)
{
    ...
    output += `<span>${getLocationName(bookings.stops[i])}</span></span></h5></div>`;
    output += `<br><hr>`;
}
output += `<div><h5>Final location:<span class="mdl-list__item-primary-content">`;
output += `<span>${getLocationName(bookings.finalLocation)}</span></span></h5></div>`;
output += `<br><hr>`;
...
function deleteCurrentBooking(index, queueIndex)
{
